...
export const getTodosByUserId = async () => {
	...
	const user = session?.user
	const userId = user?.id
	const data = await db.todo.findMany({ where: { userId } })
	return data
}
...
export const getLatestTodo = async () => {
	const session = await auth()
	const user = session?.user
	const userId = user?.id
	const data = await db.todo.findMany({ where: { userId }, take: 1, orderBy: { createdAt: "desc" } })
	return data
}
